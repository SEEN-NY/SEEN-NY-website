...
(function () {
  ...
  if (form) {
    form.addEventListener('submit', function (e) {
      ...
      window.location.href = 'mailto:tavian@equityempowerment.org?subject=' + subject + '&body=' + body;
  ...
  // ── SCROLL-TRIGGERED PAN-IN FOR SECTION HEADLINES ────────────

  function initScrollReveal() {
    const headlines = document.querySelectorAll(
      '.section-headline, .hero-headline, .tier-level, .incubator-title, .form-heading, .pillar-title, .hero-kicker, .section-label'
    );

    if (!('IntersectionObserver' in window)) {
      headlines.forEach(function (el) { el.classList.add('revealed'); });
      return;
    }
